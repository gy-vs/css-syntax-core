...
// The data types in a @property syntax descriptor are matched against tokens,
// e.g. <length> matches a <dimension-token> with a length unit. Unlike the same
// named types in property values, an unitless zero doesn't match such types.
export function createPropertySyntaxTypes(units) {
    const length = math(dimension(units.length), dimensionFunctionNames);

    return {
        'length': length,
        'length-percentage': (token, getNextToken, opts) =>
            length(token, getNextToken, opts) || productionTypes.percentage(token, getNextToken, opts)
    };
}
