...
import { checkPropertyRule, checkCustomProperties, matchPropertyValue } from './property-rule.js';
...
export class Lexer {
    ...
    // { message, node, descriptor } with the failing descriptor.
    checkPropertyRule(atrule) {
        return checkPropertyRule(this, atrule);
    }

    // Check @property rules and declarations of the registered custom
    // properties across a stylesheet AST. Values containing var() are
    // skipped since they can't be checked at build time.
    // Returns an array of error objects ({ message, node, descriptor }).
    checkCustomProperties(ast) {
        return checkCustomProperties(this, ast);
    }

    // Match a declaration value node against a syntax AST produced by
    // the @property syntax string parser. Returns true, false, or 'var'
    // when the value contains var() and can't be checked at build time.
    matchRegisteredProperty(syntax, value) {
        if (!syntax || typeof syntax !== 'object') {
            throw new SyntaxReferenceError('Bad syntax');
        }

        if (!value || (value.type !== 'Value' && value.type !== 'Raw')) {
            throw new Error('Not a Value or Raw node');
        }

        return matchPropertyValue(this, syntax, value);
    }
